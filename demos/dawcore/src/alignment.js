...
// Pure functions, no DOM/dawcore dependencies — see NOTES.md for the algorithm rationale.
...
export function detectClickOnsets(buffer, options = {}) {
...
function pairAtTrim(guideOnsets, recordedOnsets, trimGuide, trimRecorded) {
...
export function matchClickSequences(guideOnsets, recordedOnsets, options = {}) {
...
export function computeAlignmentOffset(guideBuffer, recordedBuffer, options = {}) {
  ...
  // metronome clicks (false-reliable failure mode, see NOTES.md). A count
  // mismatch is therefore disqualifying for the `reliable` proof, even
...
// Mirrors dawcore's own internal trim (packages/dawcore/src/interactions/recording-clip.ts)
// so this module always analyzes the same audible portion that ends up in the actual clip.
export function sliceBuffer(buffer, offsetSamples, durationSamples) {
