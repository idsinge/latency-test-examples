...
Note: the fork records the **raw mic stream directly** via MediaRecorder while the metronome plays separately — that capture pipeline matches the component's `recording-mode="mediarecorder-1ch"` (direct mic, start-timing bias present), likely **not** the 2-channel default `"mediarecorder"`. Confirm during investigation; this also matters for the parallel Hi-Audio integration when choosing which mode produces its compensation value.

### Hi-Audio (parallel external track)

Hi-Audio is the component's primary target. Its integration is happening **separately and in parallel** in the Hi-Audio frontend repo (by the user, using the fork above). This repo does **not** represent the Hi-Audio integration — demos here target the new libraries.

### Demo targets

1. **waveform-playlist (new React version)** — https://github.com/naomiaro/waveform-playlist (`@waveform-playlist/*` packages).
2. **dawcore** — the framework-agnostic Web Components ecosystem (`@dawcore/*`, Lit-based) emerging from the same project's migration plan: https://github.com/naomiaro/waveform-playlist/blob/main/docs/specs/web-components-migration.md. Per that spec (read 2026-06-12): worklet-based per-track recording with "latency compensation" mentioned, `daw-recording-complete` event carries `offsetSamples`, recording backend shipped but arming API not yet. The investigation is how an externally **measured** round-trip latency feeds that pipeline — the hook may already exist.
